...
PDF에서 **⌘F**를 누르면 오른쪽 위 검색창이 열립니다. 검색어를 입력하고 Enter를 누르면 검색하며, 같은 검색어에서 Enter를 다시 누르거나 이전·다음 버튼으로 강조된 검색 결과를 이동합니다. Esc로 닫습니다. 이미지로만 된 PDF는 텍스트 검색 대상이 아닙니다.
채팅 입력창에서 **Enter**는 전송, **Shift+Enter**는 커서 위치에 줄바꿈을 삽입합니다.
